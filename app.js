...
// Header text loader from local Header.xlsx (A1..A3)
async function loadHeaderTextOnce(){
  if (state.headerTextLoaded) return;
  try{
    const res = await fetch('Header.xlsx', { cache: 'no-store' });
    if (!res.ok) { state.headerTextLoaded = true; return; }
    ...
    state.headerText = {
      ...
      info: getCell('A2'),
      location: getCell('A3'),
      imageLeft: getCell('A4'),
      imageRight: getCell('A5'),
    };
  }catch(e){
    console.warn('Header.xlsx not loaded', e);
  } finally {
...
// (Template override helpers removed) – we rely solely on Template.xlsx markers when provided
...
const el = {
  ...
  preview: document.getElementById('hallPreview'),
...
function updateExamTypeInteraction() {
  ...
  // Populate subjects for selected date
  el.subjectSelect.disabled = false;
  ...
  el.subjectSelect.innerHTML = '<option value="">All subjects on date</option>' +
...
function getExamType() {
  // Since we removed exam type toggle, always use semester mode
  return 'semester';
}
...
function renderAllocations(allocations) {
  ...
  // Don't render the preview cards - just enable download buttons
  // Show success message instead
  if (el.preview) {
    const successMsg = document.createElement('div');
    successMsg.style.cssText = 'padding: 20px; background: #e8f5e9; border: 1px solid #4caf50; border-radius: 8px; color: #2e7d32; text-align: center; font-weight: 600;';
    successMsg.innerHTML = `✓ Hall allocation completed successfully!<br><small style="font-weight: normal; color: #555;">Generated ${allocations.length} hall allocation(s). Use the download buttons below.</small>`;
    el.preview.appendChild(successMsg);
  }
...
async function downloadHallPdfs() {
  console.log('downloadHallPdfs called');
  console.log('state.allocations:', state.allocations);
  ...
  if (!state.allocations || !state.allocations.length) {
    alert('No allocations available. Please allocate halls first.');
...
function onAllocate() {
  ...
  // If subject is selected, do internal allocation, otherwise semester
  if (subjectCode) {
...
el.dateSelect.addEventListener('change', updateExamTypeInteraction);
...
// ========== OVERALL ATTENDANCE FUNCTIONALITY ==========
...
function populateDepartments() {
  ...
  const depts = unique(state.students.map(s => s.dept || normalizeDeptName(inferDeptFromSubject((s.subjects||[])[0]))).filter(Boolean));
  deptSelect.innerHTML = '<option value="">Select Department</option>' +
    depts.sort().map(d => `<option value="${d}">${d}</option>`).join('');
  deptSelect.disabled = false;
}

// Generate overall attendance sheet for a department
function generateOverallAttendance() {
  console.log('generateOverallAttendance called');
  const deptSelect = document.getElementById('deptSelect');
  ...
  const deptStudents = state.students.filter(s => {
    const studentDept = s.dept || normalizeDeptName(inferDeptFromSubject((s.subjects||[])[0]));
    return studentDept === selectedDept;
  }).sort((a, b) => String(a.RegNo).localeCompare(String(b.RegNo)));
  ...
  // Get all exam dates for this department
  const deptExams = state.exams.filter(e => normalizeDeptName(e.Dept) === selectedDept || normalizeDeptName(inferDeptFromSubject(e.SubjectCode)) === selectedDept);
  const examDates = unique(deptExams.map(e => e.Date)).sort();
  
  if (examDates.length === 0) {
    alert('No exams found for this department');
    return;
  }
  
  // Update UI - show success message instead of table
  document.getElementById('deptName').textContent = selectedDept;
  
  // Clear the table and show success message
  const attendanceContainer = document.getElementById('attendanceContainer');
  const tableResponsive = attendanceContainer.querySelector('.table-responsive');
  
  if (tableResponsive) {
    tableResponsive.innerHTML = `
      <div style="padding: 40px 20px; text-align: center;">
        <div style="padding: 30px; background: #e8f5e9; border: 1px solid #4caf50; border-radius: 8px; color: #2e7d32; font-weight: 600; margin-bottom: 20px;">
          ✓ Attendance sheet generated successfully!
          <div style="font-weight: normal; color: #555; margin-top: 10px; font-size: 14px;">
            Department: <strong>${selectedDept}</strong><br>
            Students: <strong>${deptStudents.length}</strong><br>
            Exam Dates: <strong>${examDates.length}</strong>
          </div>
        </div>
        <div style="color: #666; font-size: 14px;">
          Click the "Download as PDF" button below to get the attendance sheet.
        </div>
      </div>
    `;
  }
  ...
  // Show container and enable PDF button
  attendanceContainer.classList.add('active');
  const pdfBtn = document.getElementById('exportAttendancePdfBtn');
...
function exportAttendanceToExcel() {
  ...
  if (!selectedDept) {
    alert('Please generate attendance sheet first');
    return;
  }
  ...
  // Build Excel data - removed Roll No column
  const header1 = ['S.No', 'Register No.', 'Student Name', ...examDates];
  const rows = deptStudents.map((student, idx) => {
    return [idx + 1, student.RegNo, student.Name, ...examDates.map(() => '')];
  });
  
  const footer = [
    ['', '', 'Total Present:', ...examDates.map(() => '')],
    ['', '', 'Total Absent:', ...examDates.map(() => '')],
    ['', '', 'Total OD:', ...examDates.map(() => '')],
    ['', '', 'Incharge Signature:', ...examDates.map(() => '')]
  ];
  
  const data = [header1, ...rows, ...footer];
  ...
  ws['!cols'] = [
    ...
    {wch: 15}, // Register No
    {wch: 30}, // Student Name
    ...examDates.map(() => ({wch: 15})) // Date columns with standard spacing
  ];
  
  XLSX.utils.book_append_sheet(wb, ws, selectedDept);
  XLSX.writeFile(wb, `Attendance-${selectedDept}.xlsx`);
}
...
async function exportAttendanceToPDF() {
  ...
  const selectedDept = state.currentAttendance.department;
  const deptStudents = state.currentAttendance.students;
  ...
  const body = deptStudents.map((student, idx) => {
    return [idx + 1, student.RegNo, student.Name, ...examDates.map(() => '')];
  });
  ...
  // Calculate column width for date columns to provide standard spacing
  ...
  doc.autoTable({
    ...
    columnStyles: {
      0: { cellWidth: 12 }, // S.No
      1: { cellWidth: 30 }, // Register No
      2: { halign: 'left', cellWidth: 50 }, // Student Name
      ...Object.fromEntries(examDates.map((_, i) => [i + 3, { cellWidth: dateColWidth }]))
    },
  ...
  doc.setFontSize(10);
  doc.text('Incharge Signature: _____________________', 20, finalY);
  
  doc.save(`Attendance-${selectedDept}.pdf`);
}

// Add event listeners for attendance functionality
const generateAttendanceBtn = document.getElementById('generateAttendanceBtn');
if (generateAttendanceBtn) {
  generateAttendanceBtn.addEventListener('click', generateOverallAttendance);
}

const exportAttendancePdfBtn = document.getElementById('exportAttendancePdfBtn');
if (exportAttendancePdfBtn) {
  exportAttendancePdfBtn.addEventListener('click', exportAttendanceToPDF);
}
